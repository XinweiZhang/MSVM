...
# generate_noise_circle <- function(n, noise_level){
#   draw_X <- function(y){
...
generate_noise_circle <- function(n){
  ...
  dat <- t(sapply(t, function(z){
    ...
    # }else if(z==2){
    #   xz <- runif(1, min = 7, max = 7.5)
    # }else if(z==3){
    #   xz <- runif(1, min = 14, max = 14.5)
    # }
...
for( j in 1:length(n_list)){
  clusterExport(cl=cl, varlist=c("WW_pri_opt", "Duchi_pri_opt","MDuchi_pri_opt", "CS_pri_opt","generate_noise_circle","generate_noise_circle_5class",
  ...
  acc.res <- parSapply(cl, 1:rep_n, function(s, C_list, n, sep){
    ...
    # val_data <- generate_noise_circle_5class(n)
    # 
    ...
    WW_acc <- tryCatch({
      ...
      WW_fit <- WW_pri_opt(train_data$X,train_data$y, C_list[opt_idx], intercept = F)
    ...
    CS_acc <- tryCatch({
      ...
      CS_fit <- CS_pri_opt(train_data$X,train_data$y, C_list[opt_idx], intercept = F)
    ...
    Duchi_acc <- tryCatch({
      ...
      Duchi_fit <- Duchi_pri_opt(train_data$X,train_data$y, C_list[opt_idx], intercept = F)
    ...
    MDuchi_acc <- tryCatch({
      ...
      MDuchi_fit <- MDuchi_pri_opt(train_data$X,train_data$y, C_list[opt_idx], intercept = F)
    ...
    New1_acc <- tryCatch({
      ...
      res1 <- tryCatch({ New1_fit_1 <- New1_pri_opt(train_data$X,train_data$y, C_list[opt_idx[1]], intercept = F)
      mean(predict(New1_fit_1, oracle_data$X, rule = "dagger") == oracle_data$y)}, error=function(e){
        return(NA)
      })
      res2 <- tryCatch({New1_fit_2 <- New1_pri_opt(train_data$X,train_data$y, C_list[opt_idx[2]], intercept = F)
    ...
    New3_acc <- tryCatch({
      ...
      res1 <- tryCatch({ New3_fit_1 <- New3_pri_opt(train_data$X,train_data$y, C_list[opt_idx[1]], intercept = F)
      mean(predict(New3_fit_1, oracle_data$X, rule = "dagger") == oracle_data$y)}, error=function(e){
        return(NA)
      })
      res2 <- tryCatch({New3_fit_2 <- New3_pri_opt(train_data$X,train_data$y, C_list[opt_idx[2]], intercept = F)
    ...
    LLW_acc <-  tryCatch({
      ...
      LLW_fit <- LLW_pri_opt(train_data$X,train_data$y,C_list[opt_idx], intercept = F)
    ...
    OVA_acc <-tryCatch({
      ...
      OVA_fit <- OVA_pri_opt(train_data$X,train_data$y,C_list[opt_idx], intercept = F)
    ...
    MSVM7_acc <- tryCatch({
      ...
      MSVM7_fit_1 <- MSVM7_pri_opt(train_data$X,train_data$y, C_list[opt_idx[1]], intercept = F)
      MSVM7_fit_2 <- MSVM7_pri_opt(train_data$X,train_data$y, C_list[opt_idx[2]], intercept = F)
    ...
    MSVM8_acc <-  tryCatch({
      ...
      MSVM8_fit <- MSVM8_pri_opt(train_data$X,train_data$y,C_list[opt_idx], intercept = F)
...
summary.res.arry <- apply(res.array, c(1,2), function(x){median(x,na.rm = T)})
...
save(rep_n, n_list, res.array, summary.res.arry, file = 
       paste("Noise Circle Stong Signal, rep=",rep_n,".Rdata",sep=""))
