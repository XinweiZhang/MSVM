---
title: "Another Round Experiment III"
author: "Xinwei"
date: "8/1/2020"
output: 
 html_document:
...
generate_noise_four_gaussian <- function(n, sep = 2, sep_n, train = T){
  ...
  if(train==T){
    N_n <- n/2
    t_n <- sample(seq(1,4), size = N_n, replace = T)
    dat_n <- t(sapply(t_n , function(z){
      if(z==1){
        x <- mvrnorm(1,c(1,0), sep_n^2*diag(1,2))
      }else if(z==2){
        x <- mvrnorm(1,c(0,1), sep_n^2*diag(1,2))
      }else if(z==3){
        x <- mvrnorm(1,c(0,-1), sep_n^2*diag(1,2))
      }else if(z==4){
        x <- mvrnorm(1,c(-1,0) , sep_n^2*diag(1,2))
      ...
      if(z==1){
        y = sample(seq(1,4), size=1, prob= c(1/3, 2/9, 2/9, 2/9))
      }else if(z==2){
        y = sample(seq(1,4), size=1, prob= c(2/9, 1/3, 2/9, 2/9))
      }else if(z==3){
        y = sample(seq(1,4), size=1, prob= c(2/9, 2/9, 1/3, 2/9))
      }else if(z==4){
        y = sample(seq(1,4), size=1, prob= c(2/9, 2/9, 2/9, 1/3))
      }
...
# Four-class Gaussian Weakly Separable Asymnoise
...
generate_noise_four_gaussian <- function(n, sep = 2, sep_n, train = T){
  ...
  if(train==T){
    ...
    dat_n <- t(sapply(t_n , function(z){
      ...
      if(z==1){
        y = sample(seq(1,4), size=1, prob= c(1/3, 3/9, 1/9, 2/9))
      }else if(z==2){
        y = sample(seq(1,4), size=1, prob= c(1/9, 1/2, 1/9, 5/18))
      }else if(z==3){
        y = sample(seq(1,4), size=1, prob= c(1/3, 1/9, 4/9, 1/9))
      }else if(z==4){
        y = sample(seq(1,4), size=1, prob= c(2/9, 2/9, 2/9, 1/3))
      }
