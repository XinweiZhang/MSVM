...
generate_noise_circle <- function(n, bayes){
  ...
  dat <- t(sapply(t, function(z){
    ...
    if(z==1){
      y = sample(c(1,2,3), size=1,prob= c(.55, .3, .15))
    }else if(z==2){
      y = sample(c(1,2,3), size=1,prob= c(.15, .55, .3))
    }else if(z==3){
      y = sample(c(1,2,3), size=1,prob= c(.3, .15, .55))
    }
...
generate_noise_three_circle <- function(n, sep = 2){
  ...
  dat <- t(sapply(t, function(z){
    ...
    if(z==1){
      y = sample(c(1,2,3), size=1,prob= c(.65, .15, .20))
    }else if(z==2){
      y = sample(c(1,2,3), size=1,prob= c(.20, .65, .15))
    }else if(z==3){
...
# Design 3, Filled Circle, probability 3
## sep = 1.9
```{r, fig.width=15, fig.height=9, warning=FALSE, echo=F}
...
# plot_decision_boundary(WW_fit, print = T, title = "WW")
...
# Design 3, Filled Circle, probability 4
## sep = 1.9
```{r, fig.width=15, fig.height=9, warning=FALSE, echo=F}
...
# Design 4, Gaussian, probabilty 3
## sep = 1.9
```{r, fig.width=15, fig.height=9, warning=FALSE, echo=F}
...
generate_noise_three_gaussian <- function(n, sep = 2){
  ...
  dat <- t(sapply(t, function(z){
    if(z==1){
      x <- mvrnorm(1,c(0,2), (sep*cos(1/6*pi)/3)^2*diag(1,2))
    }else if(z==2){
      x <- mvrnorm(1,c(2*cos(1/6*pi), -2*sin(1/6*pi)) , (sep*cos(1/6*pi)/3)^2*diag(1,2))
...
n = 3000
train_data <- generate_noise_three_gaussian(n, sep =1.9)
# plot_decision_boundary(WW_fit, print = T, title = "WW")
...
n = 3000
train_data <- generate_noise_three_gaussian(n, sep = 1.5)
...
n = 3000
train_data <- generate_noise_three_gaussian(n, sep = 1)
...
n = 3000
train_data <- generate_noise_three_gaussian(n, sep = .5)
...
# Design 4, Gaussian, probabilty 4
## sep = 1.9
```{r, fig.width=15, fig.height=9, warning=FALSE, echo=F}
