---
title: "New Experiment4 - Ben-David"
author: "Xinwei"
date: "9/16/2020"
output:
  html_document:
...
generate_data <- function(n, sep){
  ...
  t <- c(rep(seq(1,3), round(n*.85/3)), rep(4, round(n*.15))) 
...
```{r fig.width = 12, fig.height= 3, echo = F}
res <- sapply(seq(from = 10, to = 2)/10, function(sep){
...
```{r fig.width = 12, fig.height= 3, echo = F}
res <- sapply(c(2/3,seq(from = 6, to = 2)/10), function(sep){
  train_data <- generate_data(n_train, sep)
  test_data <- generate_data(n_test, sep)
  plot_batch(train_data, test_data,  C=1, lambda = 0, intercept = T, sep = sep)
})
