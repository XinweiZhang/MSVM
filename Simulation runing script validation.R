      ...
      parSapply(cl, 1:rep_n, function(s, C_list, oracle_data, n, sep) {
        ...
        CS_acc <- tryCatch({
          opt_idx <- which.max(sapply(C_list, function(C) {
            ...
            mean(pred(val_data$X, CS_w) == val_data$y)
          }))
        ...
        Duchi_acc <- tryCatch({
          opt_idx <- which.max(sapply(C_list, function(C) {
            ...
            mean(pred(val_data$X, Duchi_w) == val_data$y)
          }))
          Duchi_w <-
            Duchi_pri_opt(train_data$X, train_data$y, C_list[opt_idx])
        ...
        MDuchi_acc <-  tryCatch({
          opt_idx <- which.max(sapply(C_list, function(C) {
            ...
            mean(pred(val_data$X, MDuchi_w) == val_data$y)
          }))
          MDuchi_w <-
            MDuchi_pri_opt(train_data$X, train_data$y, C_list[opt_idx])
        ...
        MSVM7_acc <- tryCatch({
          opt_idx <- which.max(sapply(C_list, function(C) {
            ...
            mean(pred(val_data$X, MSVM7_w, "s") == val_data$y)
          }))
          MSVM7_w <-
            MSVM7_pri_opt(train_data$X, train_data$y, C_list[opt_idx])
        ...
        MSVM8_acc <-  tryCatch({
          ...
          MSVM8_w <-
            MSVM8_pri_opt(train_data$X, train_data$y, C_list[opt_idx])
...
summary.res.arry <-
  apply(res.array, c(1, 2, 3), function(x) {
    mean(x, na.rm = T)
  })
dimnames(summary.res.arry)[[1]] <-
  c(".5",".55",".6",".65",".7",".75",".8",".85",".9",".95",".99")
dimnames(summary.res.arry)[[3]] <-
  c("WW", "CS", "Duchi", "MDuchi", "New1", "New3", "OVA", "LLW", "MSVM7", "MSVM8")
