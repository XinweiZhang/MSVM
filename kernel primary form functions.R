WW_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel){
  ...
  objective <- Minimize(lambda*sum(do.call(rbind,sapply(1:m, FUN = function(k){quad_form(v[,k],K)})))/2 +  C*sum_entries(slack))
...
CS_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel){
  ...
  objective <- Minimize(lambda*sum(do.call(rbind,sapply(1:m, FUN = function(k){quad_form(v[,k],K)})))/2  + C*sum_entries(max_entries((1-Y)*slack, axis = 1)))
...
Duchi_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel){
  class_idx <- sort(unique(y))
  Y <- sapply(class_idx, function(id){as.numeric(y==id)})
  n <- nrow(X)
  p <- ncol(X)
...
New1_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel, base_class = NULL){
  class_idx <- sort(unique(y))
  m = length(class_idx)
  n <- nrow(X)
  ...
  K_p1 <- K[y!=base_class,,drop = F]
  K_p2 <- K[y==base_class,,drop = F]
  n_p1 <- nrow(K_p1)
  n_p2 <- nrow(K_p2)
  y_p1 <- y[y!=base_class]
  ...
  objective <- Minimize(lambda*sum(do.call(rbind,sapply(1:(m-1), FUN = function(k){quad_form(v[,k],K)})))/2 +  C*(sum(epsilon_p1)+sum_entries(slack_p2)))
...
New3_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel, base_class = NULL){
  class_idx <- sort(unique(y))
  m = length(class_idx)
  n <- nrow(X)
...
LLW_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel, base_class = NULL){
...
MSVM7_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel, base_class = NULL){
  class_idx <- sort(unique(y))
  m <- length(class_idx)
  if(is.null(base_class)){
    base_class <- m
  ...
  objective <- Minimize(lambda*sum(do.call(rbind,sapply(1:(m-1), FUN = function(k){quad_form(v[,k],K)})))/2  + C*sum_entries(slack))
...
MSVM8_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel){
...
OVA_kernel_pri_opt <- function(X, y, C = 1, lambda = 1, intercept = T, kernel){
...
plot_kernel_decision_boundary <- function(model, title = NULL, np_resolution = 500, xlim =NULL, ylim = NULL, rule = "simple_max", print = F){
