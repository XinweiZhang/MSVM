MSVM_Weak_Hard_opt <- function(X,y,type){
  ...
  }else if(type == "Duchi2")
  {
    ...
    constraints <- list(w1+w2+w3 == 0, b1+b2+b3 ==0,
                        X1 %*% (w1 - w2) + b1 - b2 >= gamma,
                        X1 %*% (w1 - w3) + b1 - b3 >= gamma,
                        X2 %*% (w2 - w1) + b2 - b1 >= gamma,
                        X2 %*% (w2 - w3) + b2 - b3 >= gamma,
                        X3 %*% (w3 - w1) + b3 - b1 >= gamma,
                        X3 %*% (w3 - w2) + b3 - b2 >= gamma,
                        gamma>=0,
                        sum(sum_squares(w1), sum_squares(w2), sum_squares(w3)) <=1)
  }else if(type == "Duchi3")
  {
    objective <- Maximize(min(X1 %*% (w1 - w2) + b1 - b2, X1 %*% (w1 - w3) + b1 - b3,X2 %*% (w2 - w1) + b2 - b1, 
...
MSVM_New_Weak_Hard_opt <- function(X,y,type,gamma){
  class_idx <- sort(unique(y))
  Y <- sapply(class_idx, function(id){as.numeric(y==id)})
  ...
  if(type == "max_of_all"){
    ...
    objective <- Minimize(sum(sum_squares(w1), sum_squares(w2), sum_squares(w3)) - gamma*sum(rho))
  }else if(type == "New1")
  {
    objective <- Minimize(sum(sum_squares(w1-w3), sum_squares(w2-w3)))
  }
  ...
  constraints <- list(w1+w2+w3 == 0, b1+b2+b3 ==0,
                      ...
                      X2 %*% (w2 - w3) + b2 - b3 >= rho[3],
                      X3 %*% (w3 - w1) + b3 - b1 >= rho[2],
                      X3 %*% (w3 - w2) + b3 - b2 >= rho[3])
...
MSVM_New2_Weak_Hard_opt <- function(X,y,type,gamma){
  class_idx <- sort(unique(y))
  Y <- sapply(class_idx, function(id){as.numeric(y==id)})
  ...
  objective <- Minimize(sum(sum_squares(w1), sum_squares(w2), sum_squares(w3)) + rho)

  
  constraints <- list(w1+w2+w3 == 0, b1+b2+b3 ==0,
                      X1 %*% (w1 - w2) + b1 - b2 >= quad_over_lin((w1 - w2), rho),
                      X1 %*% (w1 - w3) + b1 - b3 >= quad_over_lin((w1 - w3), rho),
                      X2 %*% (w2 - w1) + b2 - b1 >= quad_over_lin((w2 - w1), rho),
                      X2 %*% (w2 - w3) + b2 - b3 >= quad_over_lin((w2 - w3), rho),
                      X3 %*% (w3 - w1) + b3 - b1 >= quad_over_lin((w3 - w1), rho),
                      X3 %*% (w3 - w2) + b3 - b2 >= quad_over_lin((w3 - w2), rho))
...
MSVM_New_Weak_Hard_opt4 <- function(X,y,type, gamma){
  class_idx <- sort(unique(y))
  Y <- sapply(class_idx, function(id){as.numeric(y==id)})
  ...
  if(type == "max_of_all"){
    ...
    objective <- Minimize(sum(sum_squares(w1), sum_squares(w2), sum_squares(w3), sum_squares(w4)) -  gamma*sum(rho))
  }else if(type == "New1")
  {
...
MSVM_Weak_Hard_opt4 <- function(X,y,type){
  ...
  #                     X1 %*% (w1 - w2) + b1 - b2 >= gamma,
  #                     X1 %*% (w1 - w3) + b1 - b3 >= gamma,
  #                     X1 %*% (w1 - w4) + b1 - b4 >= gamma,
  #                     X2 %*% (w2 - w1) + b2 - b1 >= gamma,
  #                     X2 %*% (w2 - w3) + b2 - b3 >= gamma,
  #                     X2 %*% (w2 - w4) + b2 - b4 >= gamma,
  #                     X3 %*% (w3 - w1) + b3 - b1 >= gamma,
  #                     X3 %*% (w3 - w2) + b3 - b2 >= gamma,
  #                     X3 %*% (w3 - w4) + b3 - b4 >= gamma,
  #                     X4 %*% (w4 - w1) + b4 - b1 >= gamma,
  #                     X4 %*% (w4 - w2) + b4 - b2 >= gamma,
  #                     X4 %*% (w4 - w3) + b4 - b3 >= gamma,
  #                     gamma >=0,
  #                     sum(sum_squares(w1), sum_squares(w2), sum_squares(w3), sum_squares(w4)) <= 1
  # )
