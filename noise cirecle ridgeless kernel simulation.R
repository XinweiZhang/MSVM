...
for( j in 1:length(n_list)){
  ...
  acc.res <- parSapply(cl, 1:rep_n, function(s, n){
    ...
    WW_acc <- tryCatch({
      ...
      #   return(res)
      # }))
      WW_fit <- WW_kernel_pri_opt(train_data$X, train_data$y, C = 1e-5, lambda = 100, intercept = F, kernel = kernel)
    ...
    CS_acc <- tryCatch({
      ...
      #   return(res)
      # }))
      CS_fit <- CS_pri_opt(train_data$X, train_data$y, C = 1, lambda = 1, intercept = F, kernel = kernel)
    ...
    Duchi_acc <- tryCatch({
      ...
      #   return(res)
      # }))
      Duchi_fit <- Duchi_pri_opt(train_data$X, train_data$y, C = 1, lambda = 1, intercept = F, kernel = kernel)
    ...
    MDuchi_acc <- tryCatch({
      ...
      #   return(res)
      # }))
      MDuchi_fit <- MDuchi_pri_opt(train_data$X, train_data$y, C = 1, lambda = 1, intercept = F, kernel = kernel)
