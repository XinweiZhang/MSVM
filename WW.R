...
mu2 <- c(5,-5)
mu3 <- c(-5,-5)
X1 <- matrix(mvrnorm(20,mu1,matrix(c(5,0,0,5),nrow=2)), nrow =20)
X2 <- matrix(mvrnorm(20,mu2,matrix(c(5,0,0,5),nrow=2)), nrow =20)
X3 <- matrix(mvrnorm(20,mu3,matrix(c(5,0,0,5),nrow=2)), nrow =20)
X <- rbind(X1,X2,X3)
...
writeMat(con="./matlab code/WW-4class.mat", X =X, Y = y, Y_mat = Y, p=p, m=m, C = C)
...
WW <- Problem(objective, constraints)
system.time(CVXR_WW <- solve(WW, solver = "MOSEK"))
...
K <- kernelMatrix(polydot(degree = 2, scale = 1, offset = 0), X)
# K <- kernelMatrix(rbfdot(sigma = .5), X)
suppressWarnings(class(K) <- "matrix")
svd(K)$d

writeMat(con="./matlab code/WW-4class.mat", X =X, Y = y, Y_mat = Y, p=p, m=m, C = C, K = K)
C = 2^10
v <- Variable(n,m)
b <- Variable(m)
slack <- Variable(rows = n, cols = m)
objective <- Minimize(sum_squares(chol(K+diag(1e-6,nrow=nrow(K)))%*%v)/2  +  C*sum_entries(slack))
# objective <- Minimize(sum(do.call(rbind,sapply(1:m, FUN = function(k){t(v[,k])%*%K%*%v[,k]})))/2  +  C*sum_entries(slack))
...
round(CVXR_WW_v,5)
cbind(t(CVXR_WW_v)%*%X,CVXR_WW_b)
# t(cbind(t(CVXR_WW_v)%*%K,CVXR_WW_b))
