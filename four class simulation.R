...
sep_list <- c(.41, .48, .57, .65, .73, .84, .94, 1.06,  1.23, 1.45)
...
for(i in 1:length(sep_list)){
  ...
  for( j in 1:length(n_list)){
    clusterExport(cl=cl, varlist=c("WW_pri_opt", "Duchi_pri_opt","MDuchi_pri_opt", "CS_pri_opt",
                                   ...
                                   "Variable","Problem","max_entries","min_entries","pred","four_class_data_generate","mvrnorm","Maximize","vec"), envir=environment())
    acc.res <- parSapply(cl, 1:rep_n, function(s, C_list, n, sep){
      # n = n_list[j]; sep = sep_list[i];
      oracle_data <- four_class_data_generate(40000, sep = sep)
      train_data <- four_class_data_generate(n, sep = sep)
      test_data <- four_class_data_generate(n, sep = sep)
      WW_acc <- tryCatch({
      ...
      New1_acc <- tryCatch({
        opt_idx <- which.max(sapply(C_list, function(C){
          ...
          mean(pred(test_data$X,New1_w,"s") == test_data$y)
        }))
      ...
      New3_acc <- tryCatch({
        opt_idx <- which.max(sapply(C_list, function(C){
          ...
          mean(pred(test_data$X,New3_w,"s") == test_data$y)
        }))
      ...
      res <- c(WW_acc = WW_acc, CS_acc = CS_acc, Duchi_acc = Duchi_acc, MDuchi_acc = MDuchi_acc, New1_acc = New1_acc, New3_acc = New3_acc, OVA_acc = OVA_acc, LLW_acc = LLW_acc, MSVM7_acc = MSVM7_acc, MSVM8_acc = MSVM8_acc)
...
dimnames(summary.res.arry)[[1]] <- c(".5", ".55", ".6", ".65", ".7", ".75", ".8", ".85", ".9", ".95")
dimnames(summary.res.arry)[[3]] <-  c("WW","CS","Duchi","MDuchi","New1","New3","OVA","LLW","MSVM7","MSVM8")

round(summary.res.arry[,1,]*100,2)

save(sep_list, rep_n, n_list, res.array, summary.res.arry, file = 
       paste("~/Desktop/Multiclass Classification/MSVM Code/MSVM(4 class, Plan II, val same), sep=",
