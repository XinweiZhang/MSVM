...
for(i in 1:length(sep_list)){
  ...
  for( j in 1:length(n_list)){
    ...
    acc.res <- parSapply(cl, 1:rep_n, function(s, C_list, oracle_data, n, sep){
      ...
      # Duchi_pri_opt(train_data$X,train_data$y,C=1)
      Duchi_acc <- tryCatch({
        max(sapply(C_list, function(C){
...
# save(sep_list, rep_n, n_list, res.array, file = paste("~/Desktop/Multiclass Classification/MSVM Code/MSVM simulation (Plan I), sep=",length(sep_list),", n_list=",length(n_list),", rep=",rep_n,".Rdata",sep=""))
# 

summary.res.arry <- apply(res.array, c(1,2), rowMeans)
dimnames(summary.res.arry)[[2]] <- c(".5", ".55", ".6", ".65", ".7", ".75", ".8", ".85", ".9", ".95", ".99")
dimnames(summary.res.arry)[[1]] <-  c("WW","CS","Duchi","MDuchi","OVA","LLW","MSVM7","MSVM8")
  
  t(summary.res.arry[,,1])
      


t(summary.res.arry[,,3])
